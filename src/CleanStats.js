...
class CleanStats extends React.Component {
  constructor() {
    super();
    this.state = {
      weight: [],
  ...
  componentDidMount() {
    let weight = this.props.cleanStats.filter(sets => {return(sets.length > 0)}).map((session, key) => {
    ...
    let max = this.props.cleanStats.filter(sets => {return(sets.length > 0)}).map((session, key) => {
  ...
  render() {
    return (
      <div className="statspage">
        ...
        <div style={{textShadow: '3px 3px black'}}className="trackcard">
           <CardTitle >CLEAN STATS</CardTitle>
        </div>
...
const mapState = state => {
  return {
    cleanStats: state.cleanStats
  }
}

export default withRouter(connect(mapState, {})(CleanStats))
