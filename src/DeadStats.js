...
class DeadStats extends React.Component {
  constructor() {
    super();
    this.state = {
      weight: [],
  ...
  componentDidMount() {
    let weight = this.props.deadStats.filter(sets => {return(sets.length > 0)}).map((session, key) => {
    ...
    let max = this.props.deadStats.filter(sets => {return(sets.length > 0)}).map((session, key) => {
    ...
    return (
      <div className="statspage">
        ...
        <div style={{}}className="trackcard">
           <CardTitle style={{fontSize: '2.0rem'}}>DEADLIFT STATS</CardTitle>
        </div>
...
const mapState = state => {
  return {
    deadStats: state.deadStats
  }
}

export default withRouter(connect(mapState, {})(DeadStats))
