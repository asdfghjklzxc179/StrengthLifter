...
class PullupStats extends React.Component {
  constructor() {
    super();
    this.state = {
      weight: [],
  ...
  componentDidMount() {
    let weight = this.props.pullupStats.filter(sets => {return(sets.length > 0)}).map((session, key) => {
    ...
    let max = this.props.pullupStats.filter(sets => {return(sets.length > 0)}).map((session, key) => {
    ...
    return (
      <div className="statspage">
        ...
        <div style={{}}className="trackcard">
           <CardTitle >PULL-UP STATS</CardTitle>
        </div>
...
const mapState = state => {
  return {
    pullupStats: state.pullupStats
  }
}

export default withRouter(connect(mapState, {})(PullupStats))
