...
class SquatStats extends React.Component {
  constructor() {
    super();
    this.state = {
      weight: [],
  ...
  componentDidMount() {
    let weight = this.props.squatStats.filter(sets => {return(sets.length > 0)}).map((session, key) => {
    ...
    let max = this.props.squatStats.filter(sets => {return(sets.length > 0)}).map((session, key) => {
    ...
    return (
      <div className="statspage">
        ...
        <div style={{}}className="trackcard">
           <CardTitle >SQUAT STATS</CardTitle>
        </div>
...
const mapState = state => {
  return {
    squatStats: state.squatStats
  }
}

export default withRouter(connect(mapState, {})(SquatStats))
