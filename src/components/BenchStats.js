...
class BenchStats extends React.Component {
  constructor() {
    super();
    this.state = {
      weight: [],
  ...
  componentDidMount() {
    let weight = this.props.benchStats.filter(sets => {return(sets.length > 0)}).map((session, key) => {
    ...
    let max = this.props.benchStats.filter(sets => {return(sets.length > 0)}).map((session, key) => {
    ...
    return (
      <div className="statspage">
        ...
        <div style={{}}className="trackcard">
           <CardTitle >BENCH STATS</CardTitle>
        </div>
...
const mapState = state => {
  return {
    benchStats: state.benchStats
  }
}

export default withRouter(connect(mapState, {})(BenchStats))
